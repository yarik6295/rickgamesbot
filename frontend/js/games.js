...
(function () {
   ...
   */
  // Опрос состояния раунда (см. Crash.poll). Раз в POLL_MS клиент узнаёт
  // актуальную фазу с сервера — в частности, момент краша.
  const POLL_MS = 250;
  // БАГ (исправлено): между двумя опросами клиент сам досчитывал
  // мультипликатор локально (см. loop() ниже) по той же формуле, что и
  // сервер, но НЕ зная точку краша — она секретна до самого краша. Из-за
  // этого ракета иногда успевала "долететь", например, до ×1.56, хотя
  // сервер уже крашнулся на ×1.50 — и только когда приходил очередной
  // poll (раньше раз в 400мс), экран резко откатывался назад к ×1.50.
  // Чем дольше окно между опросами, тем сильнее мог быть этот перелёт
  // (при росте ×e^(0.11t) 400мс окна как раз и давали разницу порядка
  // 1.50 → 1.56, что игроки и видели).
  //
  // Настоящий источник истины — всегда сервер (он же считает выплаты),
  // но саму КАРТИНКУ можно вести так, чтобы она почти никогда не убегала
  // вперёд дальше того, что сервер уже мог обработать. Для этого рендерим
  // мультипликатор с небольшой задержкой по времени (RENDER_DELAY_MS):
  // клиент показывает не "сейчас", а "чуть раньше", то есть всегда чуть
  // отстаёт от реального роста. Если это отставание больше, чем
  // максимально возможная задержка poll'а (POLL_MS) плюс тик сервера,
  // то к моменту, когда клиент ПОКАЗЫВАЕТ какое-то значение, сервер уже
  // гарантированно успел его проверить на краш — а значит откат назад
  // становится практически невозможен (картинка теперь всегда "позади"
  // истины, а не "как получится"). Небольшая задержка в самом начале
  // полёта (первые доли секунды показывается ×1.00) — разумная плата за
  // то, что видимого рывка назад больше нет.
  ...
  const Crash = {
    ...
    // идёт по таймеру раз в POLL_MS независимо от кликов — если poll-запрос
    ...
    loop() {
      ...
      if (this.phase === 'waiting') {
        ...
        // Отстаём от реального времени на RENDER_DELAY_MS (см. комментарий
        // у константы выше) — это и есть сам фикс "перелёта" за точку краша.
        const t = Math.max(0, (now - this.flyingStartedAt - RENDER_DELAY_MS) / 1000);
  ...
  const Mines = {
    ...
    //
    // БАГ (исправлено): раньше при возврате на экран Mines старая сетка
    // (открытые клетки/мины с ПРЕДЫДУЩЕГО раунда) оставалась видна на
    // экране, пока не придёт ответ от Api.minesStatus() — сам этот запрос
    // мгновенным не бывает (реальный сетевой round-trip), и на это время
    // игрок видел чужой/устаревший результат, будто "старые мины не
    // удаляются". Теперь сетку гасим СИНХРОННО, сразу при входе на экран,
    // до отправки запроса — а уже когда ответ придёт, перерисовываем её
    // актуальным состоянием (активный раунд или чистый старт-экран).
    async resume() {
      this.active = false;
      this.revealed = [];
      this.renderGrid();
      ...
      for (let i = 0; i < this.gridSize; i++) {
        ...
        if (this.active) {
          cell.addEventListener('click', () => {
            ...
            //
            // БАГ (исправлено): раньше здесь не проверялся this.active —
            // после конца раунда (взрыв/кэшаут) сетка НЕ перерисовывается
            // намеренно (чтобы результат оставался на виду), поэтому старые
            // обработчики клика оставались навешаны на клетки. Клик по ним
            // всё ещё подсвечивал клетку классом mine-pending, хотя
            // reveal() внутри и так корректно отбрасывал такой клик по
            // active — то есть у игрока просто "подсвечивались" клетки
            // после того, как раунд уже закончился. Теперь проверка на
            // активность раунда — прямо в самом обработчике клика.
            if (!this.active || this.busy || this.revealed.includes(i)) return;
  ...
  const Towers = {
    ...
    //
    // БАГ (исправлено): при выходе с экрана Towers и возврате обратно
    // старая доска (пройденные ряды, где стояла мина, что было выбрано)
    // оставалась на экране, пока не придёт ответ Api.towersStatus() —
    // реальный сетевой запрос, не мгновенный. Игроку казалось, что "старые
    // позиции мин удаляются не сразу". Теперь доску гасим СИНХРОННО сразу
    // при входе на экран, ещё до запроса, а актуальным состоянием
    // перерисовываем уже когда оно придёт с сервера.
    async resume() {
      this.active = false;
      this.revealed = [];
      this.bombPositions = [];
        ...
        for (let t = 0; t < this.tilesPerRow; t++) {
          ...
          if (r === this.currentRow && this.active) {
            tile.addEventListener('click', () => {
              ...
              //
              // Доп. проверка this.active тут (см. тот же фикс в Mines) —
              // на всякий случай, хотя в Towers доска и так полностью
              // перерисовывается при завершении раунда (в отличие от Mines,
              // где сетка намеренно не перерисовывается сразу), так что
              // устаревшие обработчики клика тут в норме не должны
              // оставаться навешанными.
  ...
  const Upgrade = {
    ...
    async play() {
      ...
      try {
        ...
        // roll лежит в диапазоне 1..100, победа при roll <= chance (см.
        // playUpgrade на бэкенде). Зелёная зона дуги нарисована от 0° (верх
        // круга) до chance/100×360° по часовой. Раньше угол стрелки считался
        // как roll/100×360 — при roll=100 это ровно 360°, что визуально
        // совпадает с 0° (та самая точка, где НАЧИНАЕТСЯ зелёная зона), из-за
        // чего проигрышный крайний ролл выглядел так, будто стрелка попала
        // точно в выигрыш. Используем (roll-1)/100×360 — тогда roll=1 стоит
        // ровно на старте зелёной зоны, roll=chance ещё внутри нее,
        // roll=chance+1 уже точно на старте красной, а roll=100 — надёжно
        // внутри красной зоны, не наезжая на 0°/верх круга.
        const rollAngle = ((res.roll - 1) / 100) * 360;
