 ...
 * GET /api/cases/:slug
 */
function getCaseDetails(req, res) {
    const user = getOrCreateUser(req.telegramUser);
    const caseRow = db.prepare(`SELECT * FROM cases WHERE slug = ? AND is_active = 1`).get(req.params.slug);
...
function openCase(req, res) {
    ...
    const openTx = db.transaction(() => {
        ...
        const caseRow = db.prepare(`SELECT * FROM cases WHERE slug = ? AND is_active = 1`).get(slug);
        ...
        if (caseRow.price_coins > 0) {
            ...
            db.prepare(`UPDATE users SET coins_balance = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
                .run(balanceAfterDebit, user.id);
            db.prepare(`
                INSERT INTO transactions (user_id, type, amount_coins, balance_after, reference_id)
                VALUES (?, 'case_open', ?, ?, ?)
            `).run(user.id, -caseRow.price_coins, balanceAfterDebit, caseRow.id);
        }
        ...
        db.prepare(`
            INSERT INTO transactions (user_id, type, amount_coins, balance_after, reference_id)
            VALUES (?, 'case_open', ?, ?, ?)
        `).run(user.id, wonTier.value_coins, newBalance, caseRow.id);

        db.prepare(`
            INSERT INTO case_open_logs (user_id, case_id, result_item_id, server_seed, roll_value)
            VALUES (?, ?, ?, ?, ?)
        `).run(user.id, caseRow.id, wonTier.id, serverSeed, rollValue);

        if (isFreeCase) {
            db.prepare(`UPDATE users SET last_free_case_at = CURRENT_TIMESTAMP WHERE id = ?`).run(user.id);
        }

        const casesOpened = user.cases_opened + 1;
        const newLevel = computeLevel(casesOpened);
        db.prepare(`UPDATE users SET cases_opened = ?, account_level = ? WHERE id = ?`)
            .run(casesOpened, newLevel, user.id);
